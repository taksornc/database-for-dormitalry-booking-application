...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO Users (Username, PasswordHash, Email, Phone) VALUES (?, ?, ?, ?)");
        if (!$stmt->execute([$username, $passwordHash, $email, $phone])) {
...
<head>
    ...
    <style>
        ...
        input[type="text"], input[type="email"], input[type="password"], select, input[type="submit"] {
            width: 100%;
            padding: 12px;
            margin: 8px 0;
            border-radius: 4px;
